...
$row = $engine->db_query("SELECT email FROM rechargepro_account WHERE email = ? || mobile = ? LIMIT 1",array($email,$email));
$email = $row[0]['email'];
...
$devicecount = $engine->db_query("SELECT id FROM rechargepro_access WHERE email = ?",array($email),true);
if($devicecount > 2){
   echo "goo*"; exit; 
}

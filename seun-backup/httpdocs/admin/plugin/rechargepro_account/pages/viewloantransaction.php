...
<?php
$row = $engine->db_query2("SELECT transactionid,transaction_reference,rechargepro_service,rechargepro_status,amount,transaction_date,account_meter FROM rechargepro_transaction_log WHERE rechargeproid = ? AND (rechargepro_subservice = ? || rechargepro_subservice = ? || rechargepro_subservice = ?) ORDER BY transactionid DESC LIMIT 250", array($id,"loan Debit","loan Credit","loan Payment"));
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
    ...
    if($t_type == "Loan Payment"){
      $bg = "background-color:#68A24B; color:white;";  
    }   
  ?>

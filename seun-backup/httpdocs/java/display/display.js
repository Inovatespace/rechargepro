(function($) {
	$.fn.extend({
        ...
        //jQuery.fn.calllink("../index.php?p=9&m=5&cat=com&country=Nigeria");
        //jQuery.fn.callme("div","200");
		tunnel: function(options) {
			...
			return this.each(function() {
				return $(this).on('click', function(e){
					...
					$('#lightBox').append('<div id="sitecontent" class="profilebg" style="position:relative; overflow:hidden;padding:5px;"><div style="overflow:hidden;"><div class="close" style="float:right; padding-right:5px;"><img src="/java/display/images/close.png" width="25" style="cursor:pointer;" /></div></div><div id="innercontent"><img src="/java/display/images/loading.gif" /><div></div>');
					...
					$("#lightBox").center();
					$('.close').bind('click', $.proxy(function(ev) {
						$('#lightBox, #lightBoxcontainer').remove();
						ev.preventDefault();
					}, this));
		...
		calllink: function(divlink) {
			...
			//var pageheight = url.match(/height=([0-9]+)/)[1];
			$('#lightBox').append('<div id="sitecontent"><div class="close" style="float:right; padding-right:5px;"><img src="/java/display/images/close.png" /></div><div style="clear:both;"></div><div id="innercontent"><div></div>');
...
//right clickkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk
(function($, undefined) {
	$.fn.extend({

rightclick_menu: function(options, callback) {
			//Settings list and the default values
			var defaults = {
			 items:{"download": {name: "Download", icon: "edit"}},
				defaultPadding: 10,
				oddColor: '#eee',
               callback:null,
			};
        ...
        container.hide();
			...
			return this.each(function() {
				return $(this).on('contextmenu', function(e){
					...
					$("body").append('<div id="rightBox" style="position:absolute; top:'+mouseY+'px; left:'+mouseX+'px"></div>');
					$('#rightBox').append('<div id="rightBoxcontent" style="overflow:hidden;"></div>');
					//$("#rightBoxcontent").html(mouseX+"-"+ mouseY);
                            ...
                            $.each( options.items, function( i, item ){
                                ...
                                var inerhtmml = "";
                                if(i != "sep"){
                                    inerhtmml = "<div class='context' id='"+i+"'> <img style='vertical-align: middle;' src='//images/small_icons/"+item.icon+".png' width='16' /> "+item.name+"</div>";
                                }else{
                                  inerhtmml = "<div class='context-menu-separator'>&nbsp;</div>";  
                                }
                            $("#rightBoxcontent").append(inerhtmml);
                                ...
                                options.callback.call(this, {item: $(this).attr("id"), id: contextid});
                                $('#rightBox').remove();
                                })
					...
					var hideWidth = $(window).width();
					var siteWidth = $(document).width();
					var siteHeight = hideHeight - $('#rightBox').height();
					//Set heigth and width to hide to fill up the whole screen

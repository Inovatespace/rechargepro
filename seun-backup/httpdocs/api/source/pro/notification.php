...
class notification extends Api
{
    ...
    public function system_message($parameter)
    {
        ...
        $row = self::db_query("SELECT id,message,message_type FROM messages WHERE id > ? AND rechargeproid = '0' ORDER BY id DESC LIMIT 1",
            array($mid));
        ...
        $row = self::db_query("SELECT id,message,message_type FROM messages WHERE id > ? AND rechargeproid = ?  ORDER BY id DESC LIMIT 1",
            array($mid, $rechargeproid));

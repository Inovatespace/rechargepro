...
class bank_transfer extends Api
{
  ...
  "PBFPubKey": "' . $this->pub_key . '"
        ...
        $insertid = self::db_query("INSERT INTO rechargepro_transaction_log (service_charge,name,rechargeproid,ip,rechargepro_service,rechargepro_subservice,account_meter,amount,thirdPartycode,address) VALUES (?,?,?,?,?,?,?,?,?,?)",
            array(
            $myservice_charge,
            $name,
            $rechargeproid,
    ...
    public function complete_transaction($parameter)
    {
        ...
        if ($row[0]['rechargepro_status'] != "PAID") {
            ...
            self::db_query("UPDATE rechargepro_transaction_log SET service_charge=?, cordinator_id =?, rechargepro_status = ?,agent_id=?,rechargeproid=?,payment_method=?,rechargeproprofit =? WHERE transactionid = ? LIMIT 1",
                array(
                $myservice_charge,
        ...
        $jsonpost = '{"account_bank":"' . $thirdPartyCode . '","account_number":"' . $account .
            '","amount":"' . $amount . '","seckey":"' . $this->private_key .
            '","narration":"' . $address . '","currency":"NGN","reference":"' . $ref .
        ...
        if (isset($wavepost['status'])) {
            ...
            if ($wavepost['status'] == "success") {
                ...
                $result = '{"details":{"Product":"BANK TRANSFER","Account Number":"' . $account .
                    ...
                    '","Transaction fee":"' . $this->transactionfee . '","Total Amount":"' . ($amount +
                    $this->transactionfee) .
                ...
                if ($wavepost['status'] == "error") {
                    ...
                    $wavepost = self::wave_post("", "v2/gpx/transfers?seckey=" . $this->private_key .
                        "&reference=" . $ref, "GET");

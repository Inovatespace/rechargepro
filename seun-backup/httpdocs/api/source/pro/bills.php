...
class bills extends Api
{
    ...
    public function with_code($parameter)
    {
        ...
        return array(
            ...
            "bill_tertiary_field" => $bill_tertiary_field,
            "bill_need_phone" => $bill_need_phone,
            "bill_need_amount" => $bill_need_amount);
        ...
        if ($hasamount == 2) {
            if (!empty($service_resuest)) {
                ...
                if (isset($_resuest[$secondary])) {
                    ...
                    return array("status" => "100", "message" =>
                            "Biller Error, Please contact support");
                }
    ...
    public function complete_transaction($parameter)
    {
        ...
        $row = self::db_query("SELECT service_resuest,bill_return_url FROM rechargepro_services WHERE services_key = ? LIMIT 1",
            array($service));
        ...
        if (filter_var($url, FILTER_VALIDATE_URL)) {
            ...
            include realpath(dirname(dirname(__file__))) . "/billers/" . $url;
            
            $class = pathinfo($url, PATHINFO_FILENAME);
            $billclass = new $class("POST");
            $responseData = $billclass->complete_transaction($postfield);
        }
        ...
        if ($responseData['status'] == "100") {
            ...
            return array("status" => "300", "message" => $responseData['message']);
        ...
        if ($responseData['status'] == "200") {
            ...
            $responseData['Agent_name']=$namyname;

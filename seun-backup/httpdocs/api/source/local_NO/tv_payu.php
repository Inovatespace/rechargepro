...
class tv extends Api
{
    ...
    public function __construct($method)
    {
        ...
        $this->payu_payment_url = "https://mcapi-server.herokuapp.com/Vendor/SinglePayment";
        $this->payu_verify_url = "https://mcapi-server.herokuapp.com/transactions/single/";
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_POSTFIELDS => 'xml=<PayUVasRequest Ver="1.0"><MerchantId>'.$this->payu_username.'</MerchantId><MerchantReference>'.date("H:i:s").self::RandomString(4,15).'</MerchantReference><TransactionType>ACCOUNT_LOOKUP</TransactionType><VasId>MCA_ACCOUNT_SQ_NG</VasId><CountryCode>NG</CountryCode><CustomerId>'.$accountnumber.'</CustomerId></PayUVasRequest>',
...
for($i=0;$i < count($array['CustomFields']['Customfield']); $i++){
    ...
    if($array['CustomFields']['Customfield'][$i]['@attributes']['Key'] == "BASKET_ID"){
        $customerid = $array['CustomFields']['Customfield'][$i]['@attributes']['Value']; 
    }
        if($array['CustomFields']['Customfield'][$i]['@attributes']['Key'] == "FIRSTNAME"){
...
//curl_close($curl);
  ...
  $dsvt_bouquet[] = array("code"=>$response[$i]['product_key'],"invoicePeriods"=>"[1,12]","price"=>$response[$i]['amount'],"name"=>$response[$i]['bouquet_name'],"description"=>" ");
  }
  
  
    if(trim($response[$i]['bouquet_category']) == "GOTV"){
  $gotv_bouquet[] =  array("code"=>$response[$i]['product_key'],"invoicePeriods"=>"[1,12]","price"=>$response[$i]['amount'],"name"=>$response[$i]['bouquet_name'],"description"=>" ");
  }
...
$response = self::array_flatten($response);
            ...
            return array("status" => "200", "message" => array("bal"=>$myac_ballance,"pft"=>$myprofit_bal,
                    ...
                    "TransactionID" => $tid,
                    "details" => $response));
        } else if($response['ResultCode'] == "1062"){
...
$response['Total_amount'] = "N".($amount+100);
            ...
            self::db_query("UPDATE rechargepro_transaction_log SET transaction_status =?,transaction_code =?,transaction_reference =?,rechargepro_status_code =?, rechargepro_print = ? WHERE transactionid = ? LIMIT 1",
                ...
                if(!isset($parameter['sms'])){
            self::que_rechargepropay_mail($tid, $email, "success");
            self::curlit($phone, "Thank you your subscription is activated\r\nInvoice Number:".$tid."_".$rechargeproid."\r\nvisit rechargepro.com.ng, For print out");
}
$response = self::array_flatten($response);

...
class eedc extends Api
{
    ...
    public function auth_transaction($parameter)
    {
       ...
       $url = "http://eedcstaging.phcnpins.com/api/vproxy.asmx?op=FetchCust";  
...
$statusreference = $transaction_date. $tid;
  ...
  $url = "http://eedcstaging.phcnpins.com/api/vproxy.asmx?op=PostTransaction";  
$post_string = '<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
...
if (isset($response['responsecode'])) {
    ...
    if (in_array($response['responsecode'], array("01"))){
        ...
        if (!isset($response['responsecode'])) {
                ...
                if(isset($rps[0])){
                    if(in_array($rps[0],array("04","05","01"))){
...
$response = self::array_flatten($response);
        ...
        foreach (self::myarray() as $a) {
            ...
            return array("status" => "200", "message" => array("bal"=>$myac_ballance,"pft"=>$myprofit_bal,
                    ...
                    "TransactionID" => $tid,
                    "details" => $temarray));
        } else if ($response['responsecode'] == "04") {

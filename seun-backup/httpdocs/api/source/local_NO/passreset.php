<?php 
    ...
    $row = self::db_query("SELECT rechargeproid,email FROM rechargepro_account WHERE email = ? OR mobile = ?",array($email,$email));
    $row = $row[0];
    $email = $row["email"];
			...
			if (!empty($row['rechargeproid'])) {
                    ...
                    self::db_query("UPDATE rechargepro_account SET temp_code = ? WHERE rechargeproid = ?",array($token,$row['rechargeproid']));
                    ...
                    self::send_mail(array("noreply@rechargepro.com","RechargePro"),$email,"TOKEN RESET",$msg);
		...
		public static function confirm_pass_reset ($parameter){ 
          ...
          $row = self::db_query("SELECT rechargeproid,email FROM rechargepro_account WHERE 	(email = ? OR mobile = ?) AND temp_code = ? LIMIT 1",array($email,$email,$token));
    $row = $row[0];
    $rechargeproid = $row["rechargeproid"];
    
    	if (empty($row['rechargeproid'])) { return array("status"=>100,"message"=>"Invalid Code");}
    
        $password = sha1(md5($newpassword) . self::config("user_key"));
              
		$row = self::db_query("UPDATE rechargepro_account SET password = ?, temp_code='1f4' WHERE rechargeproid= ? LIMIT 1",array($password,$rechargeproid));
    ...
    if (empty($row['rechargeproid'])) { return array("status"=>100,"message"=>"Invalid Code");}
		...
		$row = self::db_query("UPDATE rechargepro_account SET temp_code='1f4' WHERE rechargeproid= ? LIMIT 1",array($rechargeproid));
        
        return array("status"=>200,"message"=>array("message"=>"successful","hash"=>md5($email)));
		}

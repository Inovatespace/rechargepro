...
if(!isset($_REQUEST['category']) || !isset($_REQUEST['network']) || !isset($_REQUEST['amount']) || !isset($_REQUEST['phone']) || !isset($_REQUEST['bundle']) || !isset($_REQUEST['nopin'])){
    ...
    switch ($category){
	case "2": //airtime
    $payload = array("service"=>$network,"amount"=>$amount,"mobile"=>$phone,"email"=>$email);
...
if(!empty($row[0]['public_secret'])){
    ...
    $payload = array("bundle"=>$bundle,"service"=>$network,"amount"=>$amount,"mobile"=>$phone,"email"=>$email);
    if($engine->get_session("rechargeproid")){
$row = $engine->db_query("SELECT public_secret FROM rechargepro_account WHERE rechargeproid = ? LIMIT 1",array($engine->get_session("rechargeproid")));  
if(!empty($row[0]['public_secret'])){
    $payload["private_key"] = $row[0]['public_secret'];
    ...
    $payload = array("nopin"=>$nopin,"service"=>$network,"amount"=>$amount,"mobile"=>$phone,"email"=>$email);
    if($engine->get_session("rechargeproid")){
$row = $engine->db_query("SELECT public_secret FROM rechargepro_account WHERE rechargeproid = ? LIMIT 1",array($engine->get_session("rechargeproid")));  
if(!empty($row[0]['public_secret'])){
    $payload["private_key"] = $row[0]['public_secret'];
...
if(isset($_REQUEST['dropdown'])){
    ...
    $vardrop = "";
$row = $engine->db_query("SELECT id,services_key,service_name FROM rechargepro_services WHERE services_category = ? AND status = '1'",array($_REQUEST['dropdown'])); 
...
type: "POST",
url: "theme/classic/pages/call/airtime.php",
data: "preview=preview&category="+agentairtime+"&network="+encodeURIComponent(network)+"&phone="+encodeURIComponent(phone)+"&bundle="+encodeURIComponent(bundle)+"&email="+encodeURIComponent(email)+"&nopin="+encodeURIComponent(nopins)+"&amount="+encodeURIComponent(amount),
cache: false,
success: function(html){
...
<div style="font-size: 140%;  margin-bottom:10px;">AIRTIME / DATA</div>
...
<select  onchange="call_network()" id="agentairtime"  class="input" style="padding:20px 1%; margin:0px; width:99%;" ><option value="" hidden="hidden">Select Airtime / Data</option>
...
<input id="phone" class="input" style="padding:20px 1%; margin:0px; width:99%;" autocomplete="off" type="text" placeholder="Enter Phone/Account Number"/>
...
<select onchange="change_data()" class="input" id="network" style="padding:20px 1%; margin:0px;  width:99%;" ><option value="" hidden="hidden">Select Network</option>
</select>
<div class="selectdrop fa fa-spinner fa-spin" style="display:none; position: absolute; right:30px; top:8px;"></div>
</div>
  
  
  <div class="nopinsholder" style="overflow: hidden; margin-bottom:10px;" >
...
<div class="inputholder  inputholder2" style="overflow: hidden; margin-bottom:10px; position: relative;" >
<input id="amount" class="input" style="padding:20px 1%; margin:0px; width:99%;" type="text" placeholder="Enter Amount"/>
<select class="input" id="bundle" style="display:none; padding:20px 1%; margin:0px; width:99%;" ><option value="" hidden="hidden">Select Bunble</option>
</select>
<div class="selectdrop fa fa-spinner fa-spin" style="display:none; position: absolute; right:30px; top:8px;"></div>

<div style="margin-top:10px;">Token/Receipt will be sent to the email/phone entered above</div>
</div>

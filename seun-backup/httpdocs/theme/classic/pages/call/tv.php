...
$payload = array("accountnumber"=>$meter,"service"=>$cabletv,"amount"=>$tmpamt[1],"mobile"=>$phone,"email"=>$email,"code"=>$code);
...
$.ajax({
    ...
    success: function (html){
        ...
        if (typeof html['message']['items'] === "undefined") {
            load_amount();
            }else{
        var dropdown = '';
...
var datatosend = "cabletv="+encodeURIComponent(cabletv)+"&meter="+encodeURIComponent(meter)+"&amount="+encodeURIComponent(cabletvtext)+"&code="+code+"&phone="+encodeURIComponent(phone)+"&email="+encodeURIComponent(email)+"&preview=preview";
...
<select onchange="load_amount()" id="cabletv" class="input" style="padding:20px 1%; margin:0px; float:left; width:99%;" >
...
<input id="meter" class="input" style="padding:20px 1%; margin:0px; float:left; width:99%;"  placeholder="Enter Smart Card Number"/>
</div>
...
<select id="amount" class="input" style="padding:20px 1%; margin:0px; float:left; width:99%;" >
<option  value="" hidden="hidden">Select Network</option>
</select>
...
<div style="margin-bottom:10px;">Invoice will be sent to the email address entered above</div>

...
if(!empty($_REQUEST['subject']) && !empty($_REQUEST['message'])){
        ...
        $row = $engine->db_query("SELECT rechargeproid FROM appauth WHERE authkey = ?  LIMIT 1",
            array($key)); //AND status = '0'
        $rechargeproid = $row[0]['rechargeproid'];


        $row = $engine->db_query("SELECT public_secret,email, name FROM rechargepro_account WHERE rechargeproid = ? AND active = '1' LIMIT 1",
            array($rechargeproid));
        $private_key = 

...
class myapp extends Api
{
       ...
       curl_setopt($ch, CURLOPT_URL,'https://api.ravepay.co/flwv3-pug/getpaidx/api/v2/verify');
        ...
        if ($response["data"]["chargecode"] == "00") {
            ...
            if (isset($response["data"]["flwRef"])) {
                ...
                $row = self::db_query("SELECT rechargeproid,ac_ballance,profit_bal FROM rechargepro_account WHERE public_secret = ? LIMIT 1",
                    array($private_key));
                $rechargeproid = $row[0]['rechargeproid'];
                ...
                if (!empty($transactionid)) {
                    return array("status" => 200, "message" =>array("bal"=>$ac_ballance,"pft"=>$profit_bal));
                }
                ...
                return array("status" => 200, "message" => array("bal"=>$newbalance,"pft"=>$profit_bal));
    ...
    public function ac_ballance($parameter)
    {
        ...
        $row = self::db_query("SELECT ac_ballance,profit_bal, rechargeproid FROM rechargepro_account WHERE public_secret = ? AND active = '1' LIMIT 1",
            array($parameter['private_key']));
    ...
    public function history($parameter)
    {
        ...
        if (!empty($search)) {
            ...
            $content = array();
            $row = self::db_query("(SELECT rechargeproid,transaction_reference, amount, phone, rechargepro_service,transaction_date,rechargepro_status,transactionid,rechargepro_subservice, rechargepro_status_code, account_meter FROM rechargepro_transaction_log WHERE	rechargeproid  = ? AND rechargepro_status = 'PAID'  ORDER BY transaction_date DESC LIMIT $start, $per_page)",
                array($rechargeproid));
        }

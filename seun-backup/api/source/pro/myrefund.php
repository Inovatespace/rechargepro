...
class myrefund extends Api
{
    ...
    function refund_now($parameter)
    {
        ...
        $row = self::db_query("SELECT rechargepro_print,transaction_date,account_meter,phone,rechargepro_service,cordinator_id,agent_id,rechargeproid,rechargepro_subservice,amount,thirdPartycode,refererprofit,agentprofit,cordprofit,rechargeproprofit,rechargepro_service_charge FROM rechargepro_transaction_log WHERE transactionid = ? AND refund = '0' AND bank_ref ='' LIMIT 1",
            array($tid));
        ...
        $rechargeproprofit = $row[0]['rechargeproprofit'];
        ...
        if (!empty($account_meter) && $rechargepro_subservice != "BANK TRANSFER") {
            ...
            $row = self::db_query("SELECT merge_ac,profit_bal,rechargeprorole, ac_ballance, profile_creator , name, email FROM rechargepro_account WHERE rechargeproid = ? LIMIT 1",
                array($rechargeproid));
        ...
        if ($rechargepro_subservice == "BANK TRANSFER") {
            ...
            $tfee = 15 + $rechargeproprofit;
            $himballance = $myballance + $amount_to_charge + $tfee;
            self::db_query("UPDATE rechargepro_account SET ac_ballance = ? WHERE rechargeproid = ? LIMIT 1",
                array($himballance, $rechargeproid));
        }

...
class irecharge extends Api
{
    ...
    public function auth_transaction($parameter)
    {
        ...
        $combined_string = $this->vendor_code . "|" . $reference_id . "|" . $accountnumber .
            "|" . $this->pub_key;
        $hash = hash_hmac("sha1", $combined_string, $this->priv_key);
        //create URL
        $url = "https://irecharge.com.ng/pwr_api_sandbox/v2/get_meter_info.php?vendor_code=" .
            urlencode($this->vendor_code) . "&meter=" . urlencode($accountnumber) .
            "&reference_id=" . urlencode($reference_id) . "&disco=" . urlencode($disco) .
            "&response_format=json&hash=" . urlencode($hash);

...
//file_put_contents("veveveee.php",$result);
        ...
        $response = strtolower($array['body']['fetchcustresponse']['fetchcustresult']);
        ...
        //00_AccountNo: 43012546826, AccountType: 1, Address: Physical, 52 TOKOTAYA AMURE ESTATE LOWA ESTATE IKORODU, Balance: 0.00, ContactNo: Cell:08068973623, MeterNo: 43012546826, MinAmount: 9,747.84, Name:      MR STEPHEN
        ...
        if (empty($re)) {
            return array("status" => "100", "message" =>
                    "Invalid meter details or network Error, try again5");
        }
        ...
        $thirdParty = "";
        if (isset($exp[1])) {
            $ex = explode("00_accountno:", $response);
            $thirdParty = $ex[1];
        }
        ...
        $address = "";
        if (isset($exp[2])) {
            $ex = explode("address:", $response);
            $address = $ex[1];
            
            if(isset($ex[1])){
             $ex = explode("balance:", $ex[1]);   
             $address = $ex[0];
            }
 ...
 ////////////////////////////////// AUTO FEED END
        ...
        $url = "https://www.iepins.com.ng/API/vproxy.asmx?op=PostTransaction3";
        $post_string = '<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <PostTransaction3 xmlns="http://IKEDC_API/vproxy/">
      <AccountNo>' . $accountnumber . '</AccountNo>
      <amount>' . $amount . '</amount>
      <hashstring>' . md5($accountnumber . "IE5273") . '</hashstring>
      <api_key>E146A2C4460B6511AEA043565D605C6A</api_key>
      <phone>' . $phone . '</phone>
      <txnID>' . $statusreference . '</txnID>
      <repro>1</repro>
    </PostTransaction3>
  </soap:Body>
</soap:Envelope>';
       ...
       file_put_contents("nakejo.php",$result);
        ...
        if (!isset($array['body'])) {
            return array("status" => "100", "message" =>  "Invalid meter details or network Error, try again1");
        }


        if (!isset($array['body']['posttransaction3response'])) {
            return array("status" => "100", "message" =>"Invalid meter details or network Error, try again2");
        }


        if (!isset($array['body']['posttransaction3response']['posttransaction3result'])) {
            return array("status" => "100", "message" =>  "Invalid meter details or network Error, try again3");
        }


        $mainresponse = $array['body']['posttransaction3response']['posttransaction3result'];
        
        //$mainresponse = '00_Successful|CreditToken:14831761798352312960|TranId:201910311617070737|MeterNo:54181256584|Rate:0.05|Value:21.8. Thank you for using the service. Ikeja Electric.';
        
        
        
        //00_Your transaction was unsuccessful. Insufficient Amount. Minimum amount due: 9,747.84. TranId: 0000. For help, call 014483900, 07000225543. Ikeja Electric.
        file_put_contents("nakejo2.php",$mainresponse);
        
        
            $ex = explode("_",$mainresponse);
            $response['responsecode'] = $ex[0];
            $response['responsemessage'] =  $ex[1];
            
            
            
            if (strpos($ex[1], 'unsuccessful') !== false) {
                include "refund.php";
...
//00_Successful|CreditToken:14831761798352312960|TranId:201910311617070737|MeterNo:54181256584|Rate:0.05|Value:21.8. Thank you for using the service. Ikeja Electric.
       
            
            
       file_put_contents("nakejo3.php",json_encode($response));
            ...
            if ($response['responsecode'] == "04") {
                    ...
                    if (in_array($response['responsecode'], array("01"))) {
                        ...
                        if ($myrefund == "200") {
                            ...
                            if ($myrefund == "300") {
                                ...
                                return array("status" => "100", "message" => "Transaction Reversedee");
                            }
                ...
                if ($response['responsecode'] == "00") {
                    ...
                    if ($myrefund == "200") {
                        ...
                        if ($myrefund == "300") {
                            ...
                            return array("status" => "100", "message" => "Transaction Reversedff");
                        }
                ...
                if ($myrefund == "200") {
                    ...
                    if ($myrefund == "300") {
                        ...
                        return array("status" => "100", "message" => "Transaction Reversedd");
                    }
  ...
  <soap:Body>
    <FetchTxnByRef xmlns="http://localhost/eedc/vproxy/">
      ...
      <api_key>E146A2C4460B6511AEA043565D605C6A</api_key>
        ...
        file_put_contents("nakeju.php",$result);

...
class percentage extends Api
{
    ...
    function calculate_per($parameter)
    {
        ...
        if ($rechargeproid == 0) {
            $percent = ($bill_rechargeprofull_percentage) - (($amount * 1.5) / 100);
            if ($bill_formular == 0) {
                $percent = (($amount * ($bill_rechargeprofull_percentage)) / 100) - (($amount *
                    1.5) / 100);
            }
            ...
            //////////////////////////////
            if ($payment_method == "2") {
                $rowb = self::db_query("SELECT merge_ac,ac_ballance, rechargeproid, profile_creator, rechargepro_cordinator, rechargeprorole FROM rechargepro_account WHERE rechargeproid = ? AND active = '1' LIMIT 1",
                    array($rechargeproid));
                ...
                if ($rechargeprorole > 3) {
                    ...
                    if ($profile_creator > 0) {
                        ...
                        if ($bill_formular == 1) {
                            $special_percent = ($amount * 5) / 100;//specialamount
                        }
                        ...
                        if ($referearprofit > 0) {
                            ...
                            self::update_reward($itd, $rechargeproid, $profile_creator, $referearprofit,
                                "REWARD");
                ...
                if ($rechargepro_cordinator > 0 || $rechargeprorole == 1) {
                    ...
                    self::update_reward($itd, $rechargeproid, $rechargepro_cordinator, $corprofit,
                        "REWARD");

                    $row = self::db_query("SELECT profit_bal,merge_ac,ac_ballance FROM rechargepro_account WHERE rechargeproid = ? LIMIT 1",
                        array($rechargepro_cordinator));
                    $cordinator_ballance = $row[0]['profit_bal'];
                    $merge_ac = $row[0]['merge_ac'];
                    $cordinator_ac_ballance = $row[0]['ac_ballance'];
                    ...
                    if ($merge_ac == 0) {
                        ...
                        $cornewballance = $cordinator_ac_ballance + $corprofit;
                        self::db_query("UPDATE rechargepro_account SET ac_ballance = ? WHERE rechargeproid = ? LIMIT 1",
                            array($cornewballance, $rechargepro_cordinator));
                    }
                }
                ...
                if ($rechargeprorole < 4) {
                    ...
                    $merge_ac = $row[0]['merge_ac'];
                    $ownerballance_ac_ballance = $row[0]['ac_ballance'];
                    ...
                    if ($merge_ac == 0) {
                        ...
                        $newballance = ($ownerballance_ac_ballance - $fixedfee) + $agentprofit;
                        self::db_query("UPDATE rechargepro_account SET ac_ballance = ? WHERE rechargeproid = ? LIMIT 1",
                            array($newballance, $rechargeproid));
                    }
                ...
                if ($rechargeprorole < 4) {
                    ...
                    if ($bill_formular == 0) {
                        $percent = (($amount * $bill_rechargeprofull_percentage) / 100);
                    }
                ...
                //add user percentage and reduce rechargepro
                $promopercet = 0;
                if (in_array($services_category, array(1, 2)) && $promo1 > 0 && $rechargeprorole >
                    3) {
    ...
    public function update_reward($itd, $rechargeproid, $rechargepro_cordinator, $amount,
        $wht = "PROFIT")
    {

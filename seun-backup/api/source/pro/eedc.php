  ...
  <soap:Body>
    <FetchCust xmlns="http://localhost/eedc/vproxy/">
      ...
      <api_key>579B5A722C993135F3E0F906AB84FBBE</api_key>
        ...
        file_put_contents("eedcp.php",$result);
        ...
        $response = json_decode($array['body']['fetchcustresponse']['fetchcustresult'], true);


        //"04" invalid input
        //04 exist
        //01 You have exceeded your Prepaid vending limit


        //{"responsecode":"00","responsemessage":"Customer Not Found","Name":"Account/Meter No. not found","address":null,"ContactNo":null,"":null,"MeterNo":null,"accounttype":0,"district":null,"customerid":-2}
        ...
        if ($response['accountno'] == null) {
            return array("status" => "100", "message" => "Invalid meter");
        }
        ...
        $thirdParty = "";
        $business = "";
        $thirdParty = $response['accounttype'];
        $unique = $response['customerid'];
    ...
    public function complete_transaction($parameter)
    {
        ...
        $url = "http://eedc.phcnpins.com/api/vproxy.asmx?op=PostTransaction";
        $post_string = '<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <PostTransaction xmlns="http://localhost/eedc/vproxy/">
      ...
      <txnref>' . $statusreference . '</txnref>
        ...
        if (!isset($array['body']['posttransactionresponse'])) {
            return array("status" => "100", "message" =>
                    "Invalid meter details or network Error, try again2");
        }


        if (!isset($array['body']['posttransactionresponse']['posttransactionresult'])) {
            return array("status" => "100", "message" =>
                    "Invalid meter details or network Error, try again3");
        }

        $mainresponse = $array['body']['posttransactionresponse']['posttransactionresult'];
            ...
            if (!empty($mainresponse)) {
                ...
                if (isset($rps[0])) {
                    if (in_array($rps[0], array(
                        ...
                        "05",
                        "01"))) {
                        return array("status" => "100", "message" => $mainresponse . "An error occured");
                    }
                }
            }
    ...
    function verify_transaction($tref)
    {
        ...
        $url = "http://eedc.phcnpins.com/api/vproxy.asmx?op=FetchTxnByRef";
        $post_string = '<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>

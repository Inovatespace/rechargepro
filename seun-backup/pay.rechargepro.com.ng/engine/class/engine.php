...
function url_origin($use_forwarded_host = false )
{
    ...
    function notification($quickpayid_main, $message, $type = 0)
    {

        $vowels = array("<br />", "<br/>");
        $message = str_replace($vowels, "", $message);

        self::db_query("INSERT INTO messages (quickpayid,message,message_type) VALUES (?,?,?)",
            array(
            $quickpayid_main,
            $message,
            $type));
    ...
    public function que_quickpaypay_mail($tid, $email, $status)
    {
        //validate email
        if ($status == "success") {
            ...
            $row = self::db_query("SELECT quickpay_status,transactionid,quickpay_subservice,account_meter,phone,quickpay_service,amount,transactionid,business_district,thirdPartycode,address,name,phcn_unique,quickpay_status_code,quickpay_print FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
                array($tid));
          ...
          //  }
            ...
            if (!empty($tosend)) {
                return self::send_mail("noreply@quickpaypay.com", $email, "quickpaypay $service Purchase",
                    $tosend);
            } else {
                return "ok";
            }
    ...
    public function curlit($from, $message, $sender="QuickPay")
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://api.smartsmssolutions.com/smsapi.php");
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            "username=Cvnigboka@gmail.com&password=Shalla12&sender=$sender&recipient=$from&message=$message");
        // Set a referer
        curl_setopt($ch, CURLOPT_REFERER, "http://quickpay.com.ng");
        // User agent
    ...
    public function que_quickpaypay_sms($tid)
    {
        if (!isset($tid)) {
            return "ok";
        }
    ...
    {
        self::db_query("INSERT activity_log (quickpayid,activity_type,who,event,ip) VALUES (?,?,?,?,?)",
            array(
            $id,
            $type,
            $who,
    ...
    private function body($call = 1)
    {
        ...
        if ($call == 2) {
            ...
            if (file_exists(self::config("theme_folder") . self::config("theme") . "/pages/" .
                ...
                $row = self::db_query("SELECT username FROM quickpay_account WHERE username = ? LIMIT 1",
                    array($request));
...
';
    ...
    function account_menu($page)
    {
        ...
        $ac = "";
        $acl = "";
        if (self::get_session("quickpayrole")) {
...
<div>Setting</div>
            ...
            if (self::get_session("quickpayrole") < 3) {

...
$row = $engine->db_query("SELECT transactionid,quickpay_service,quickpay_subservice,account_meter,business_district,thirdPartycode,address,name,phcn_unique,amount,phone,email,payment_method,transaction_status,transaction_code,transaction_reference,quickpay_status,quickpay_status_code,quickpay_print,transaction_date FROM quickpay_transaction_log WHERE transactionid = ?",array($id));
...
?>
<?php
$row = $engine->db_query("SELECT id FROM quickpay_services WHERE services_key = ?",array($quickpay_subservice));
?>
...
<div class="nextcolor" style="border-bottom:1px #CCCCCC solid; border-top:1px #CCCCCC solid; font-size:25px; padding: 20px 0px; font-weight:bold;"><?php echo $quickpay_service;?> RECEIPT</div>
...
<?php
 $array = $engine->myarray();
        ...
        if(in_array($quickpay_subservice,array("2351","2352","2353","2354"))){
...
<?php
    ...
    if(isset($_REQUEST['tid'])){
        ...
        <div><strong>Transaction ID</strong><?php echo htmlentities($_REQUEST['quickpay']);?></div>

...
if($quickpayrole > 2){ echo "<meta http-equiv='refresh' content='0;url=/transactionlog'>"; exit;};
...
function updaclassicatus(Id,what){
	...
	if(my_proile != Id){
            ...
            $.ajax({
                ...
                data: "tquickpayid="+Id+"&status="+status+"&what="+what,
                cache: false,
                success: function (html) {
...
<div class="" style="margin-bottom: 10px; padding:10px 20px; overflow:hidden; background-color:white;">
...
if(isset($_SESSION['main'])){
    if($_SESSION['main'] == 2){
     ...
     ?>
<button style="cursor:pointer; float: right; border: none; padding:5px 10px; margin:3px;"  name="theme/classic/pages/profile/pro/newprofile.php?width=300" class="tunnel mainbg shadow"><span class="fas fa-user-tie"></span> New Agent</button>
<?php   
    }
}else{
?>
...
switch ($quickpayrole){
	case "1":
    $rowcount = $engine->db_query("SELECT quickpayid FROM quickpay_account WHERE profile_agent = ? ", array($profile_creator), true);
	break;
   ...
   $rowcount = $engine->db_query("SELECT quickpayid FROM quickpay_account WHERE profile_agent = ?", array($profile_creator), true);
    ...
    case "3":
   $rowcount = $engine->db_query("SELECT quickpayid FROM quickpay_account WHERE profile_creator = ?", array($profile_creator), true);
	break;
...
<?php }	if($rowcount < 1){  echo "<div style='padding:5%; margin:5%; font-size:200%;  overflow:hidden;'>No Agent have been added to this account</div>";}?>
...
<div style="font-size: 180%; display: none;">Agent Commission</div>
<div style="overflow-x:auto; max-height: 300px; margin-top: 20px; font-size: 80%; display: none;">
...
$row = $engine->db_query("SELECT id,services_key,cordinator_percentage,percentage,bill_formular,dateadeded,bill_quickpayfull_percentage FROM quickpay_services_agent WHERE quickpayid = ?",array($profile_creator));
$sub = array();
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
  ...
  $myservicearray[$services_key] = array($id,$services_key,$percentage,$bill_formular,$dateadeded,$cordinator_percentage,$bill_quickpayfull_percentage);  
    
    }
    
$mychargearray = array();
$row = $engine->db_query("SELECT id,services_key,fixedfee,dateadeded FROM quickpay_services_fixed WHERE quickpayid = ?",array($profile_creator));
$sub = array();
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
    $id = $row[$dbc]['id']; 
    $services_key = $row[$dbc]['services_key']; 
...
$row = $engine->db_query("SELECT quickpayservice_charge,bill_quickpayfull_percentage,cordinator_percentage,id,services_key,service_name,percentage,bill_formular,services_category,status,dateadeded FROM quickpay_services WHERE services_category != '7' ORDER BY id DESC LIMIT 500",array());
$sub = array();
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
    ...
    $cordinator_percentage = $row[$dbc]['cordinator_percentage'];
    $bill_quickpayfull_percentage = $row[$dbc]['bill_quickpayfull_percentage'];
    $quickpayservice_charge = $row[$dbc]['quickpayservice_charge'];
    ...
    $bill_quickpayfull_percentage = $myservicearray[$services_key][6];
    }
    ...
    $quickpayservice_charge = $mychargearray[$services_key][2];  
...
<th><?php if($quickpayrole == "1"){ echo ($cordinator_percentage+$percentage).$what;}else{echo $percentage.$what;}?></th>
<td><?php echo $quickpayservice_charge;?></td>

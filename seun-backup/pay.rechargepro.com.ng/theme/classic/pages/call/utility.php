<?php
if(!isset($_SESSION)){require "../../../../engine.autoloader.php";}
...
$array = array();
$row = $engine->db_query("SELECT services_key FROM quickpay_services WHERE services_category = ?  AND status = '1'",array(1)); 
...
$array[] = $services_key;
   ...
   $row = $engine->db_query("SELECT transactionid,quickpayid FROM quickpay_transaction_log WHERE quickpay_subservice IN ($implode)  AND quickpay_status = 'PAID' ORDER BY transactionid DESC LIMIT 1",array());  
    $transactionid = $row[0]['transactionid'];
    
    if(empty($transactionid)){ echo "bad"; exit;}
    echo  $row[0]['quickpayid']."_".$transactionid;exit;
    }
...
$payload = array("accountnumber"=>$meter,"service"=>$utility,"amount"=>$amount,"mobile"=>$phone);
...
$("#sendutility").html('<i class="fa fa-spinner fa-spin"></i> Loading');    
    ...
    var datatosend = "utility="+encodeURIComponent(utility)+"&meter="+encodeURIComponent(meter)+"&amount="+encodeURIComponent(amount)+"&phone="+encodeURIComponent(phone)+"&preview=preview";
...
<select id="utility" class="input" style="padding:20px 1%; padding-left:50px; margin:0px; float:left; width:99%;" >
<option  value="" hidden="hidden">Select Utility</option>
<?php
$row = $engine->db_query("SELECT id,services_key,service_name FROM quickpay_services WHERE services_category = ?  AND status = '1' ORDER BY id",array(1)); 
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
    $id = $row[$dbc]['id'];
    $services_key = $row[$dbc]['services_key'];
...
<input id="meter" class="input" style="padding:20px 1%; padding-left:50px; margin:0px; float:left; width:99%;"  placeholder="Enter Meter Number"/>
</div>
...
<div style="overflow: hidden;">
<div class="inputholder inputholder2" style="overflow: hidden; margin-bottom:10px;" >
<input id="amount" class="input" style="padding:20px 1%; padding-left:50px; margin:0px; float:left; width:99%;" type="number" placeholder="Enter Amount"/>
</div>
...
<div style="margin-bottom:10px;">Token will be sent to the phone number entered above</div>
...
<input id="tel" class="input" style="padding:20px 1%; padding-left:50px; margin:0px; margin-bottom:10px; width:100%;" type="tel" placeholder="Enter Phone Number"/>

<button class="mainbg" id="recover" onclick="recover()" style="cursor:pointer; border:none; padding:20px 1%; padding-left:50px; margin:0px; float:left; width:100%; margin-bottom:10px; color:white;">Recover</button>

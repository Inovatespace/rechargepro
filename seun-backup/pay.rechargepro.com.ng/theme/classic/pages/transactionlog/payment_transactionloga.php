...
$.ajax({
type: "POST",
url: "<?php echo $engine->config("website_root")."api/local/myapp/try_again_new.json";?>",
data: "tid="+Id,
cache: false,
...
<?php
$row = $engine->db_query("SELECT services_key, service_name FROM quickpay_services WHERE services_category < '7'",array());
...
$row = $engine->db_query("SELECT quickpayid,name FROM quickpay_account WHERE quickpayid = ?",array($engine->get_session("quickpayid")));
}
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
...
$rowcount = $engine->db_query("SELECT agent_id FROM quickpay_transaction_log WHERE $call  AND quickpay_status = 'PAID'", array(), true);
...
<div id="page-content"><div style="text-align:center; padding:20px;"><img src="/theme/classic/images/quickpay.gif" width="124" height="124" /></div></div>

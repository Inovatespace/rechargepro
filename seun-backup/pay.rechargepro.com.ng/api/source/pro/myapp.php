...
class myapp extends Api
{
    ...
    {
        $cat = $parameter['cat'];
        $row = self::db_query("SELECT services_key,service_name FROM quickpay_services WHERE services_category = ? AND service_subcategory =?  AND status = '1' ORDER BY service_name ASC",
            array(7, $cat));
        $return = array();
    ...
    {
        $row = self::db_query("SELECT subcategory_id,name FROM quickpay_subcategory ORDER BY name ASC",
            array());
        $return = array();
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
    ...
    public function saveprofile($parameter)
    {
        ...
        $row = self::db_query("SELECT quickpayid FROM appauth WHERE authkey = ? AND quickpayid = ? AND status = '0' LIMIT 1",
            array($parameter['private_key'], $id));
        $quickpayid = $row[0]['quickpayid'];


        $row = self::db_query("SELECT transfer_activation FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
            array($quickpayid));
        ...
        self::db_query("UPDATE quickpay_account SET name =?, bank_name=?, bank_ac_name= ?, bank_ac_number= ? WHERE quickpayid = ? LIMIT 1",
            array(
            $name,
            $bankname,
            $acname,
    ...
    public function savepassword($parameter)
    {
        ...
        $rowa = self::db_query("SELECT quickpayid,password FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
            array($quickpayid));
        $row = $rowa[0];
        ...
        self::db_query("UPDATE quickpay_account SET password =? WHERE quickpayid = ? LIMIT 1",
            array($password, $row['quickpayid']));
    ...
    public function banquet_list($parameter)
    {
        ...
        $row = self::db_query("SELECT quickpayid FROM appauth WHERE authkey = ?  LIMIT 1",
            array($key)); //AND status = '0'
        $quickpayid = $row[0]['quickpayid'];
                $row = self::db_query("SELECT profile_agent FROM quickpay_account WHERE quickpayid = ? AND active = '1' LIMIT 1",
            array($quickpayid));
    ...
    public function auth($parameter)
    {
        ...
        $row = self::db_query("SELECT email FROM quickpay_account WHERE email = ? || mobile = ? LIMIT 1",
            array($email, $email));
        $email = $row[0]['email'];
        ...
        $devicecount = self::db_query("SELECT id FROM quickpay_access WHERE email = ?",
            array($email), true);
        if ($devicecount > 2) {
            return "goo";
        }
        ...
        self::db_query("INSERT INTO quickpay_access (email,mac,name,device_type) VALUES (?,?,?,?)",
            array(
            $email,
            $serial,
            $devicename,
    ...
    public function login($parameter)
    {
        ...
        //self::db_query("INSERT INTO quickpay_account (companyaddress) VALUES (?)", array($money));
        //$hasher = new PasswordHash(8, false);
        $latestpassword = sha1(md5($password) . self::config("user_key"));


        $rowa = self::db_query("SELECT quickpayid,name,password,quickpayrole,public_secret,email,ac_ballance,sms_activation,transfer_activation,bypass,profit_bal FROM quickpay_account WHERE (mobile = ? OR email = ?) AND active = '1' LIMIT 1",
        ...
        if (!self::CheckPassword($password, $row['password'])) {
            ...
            //check if mbc_access
            $accesspacount = 0;
            $rowm = self::db_query("SELECT password FROM quickpay_access WHERE email = ? AND mac = ?",
        ...
        if (empty($row['quickpayid'])) {
            ...
            if ($deviceisfree == 4233333) { //0
                ...
                $regdevice = 0;
                $isallowed = 0;
                $rowb = self::db_query("SELECT device_type,id,email,mac,name FROM quickpay_access WHERE email = ?",
                    array($email));
                ...
                $rowb = self::db_query("SELECT id FROM quickpay_access WHERE email = ? AND mac = ?",
                    array($email, $serial));
                if (empty($rowb[0]['id'])) {
                    self::db_query("INSERT INTO quickpay_access (email,mac,name,device_type) VALUES (?,?,?,?)",
                        array(
            ...
            self::db_query("UPDATE appauth SET status = '1' WHERE quickpayid = ? AND accesstype = ?",
                array($row['quickpayid'], $type));
            $code = self::RandomString(4, 30);

            $ip = self::getRealIpAddr();
            self::db_query("INSERT INTO appauth (quickpayid,email,name,authkey,accesstype,ip) VALUES (?,?,?,?,?,?)",
                array(
                $row['quickpayid'],
                $row['email'],
                $row['name'],
            ...
            return $row['quickpayid'] . "@" . $row['name'] . "@" . $row['quickpayrole'] .
    ...
    public function register($parameter)
    {
        ...
        $row = self::db_query("SELECT email,mobile FROM quickpay_account WHERE email = ? || mobile = ? LIMIT 1",
            array($email, $mobile));
        ...
        $creator = "0";
        if (!empty($referer)) {
            $row = self::db_query("SELECT quickpayid FROM quickpay_account WHERE email = ? || mobile = ? LIMIT 1",
                array($referer, $referer));

            if (!empty($row[0]['quickpayid'])) {
                $creator = $row[0]['quickpayid'];
            }
        }


        $quickpayid = self::db_query("INSERT INTO quickpay_account (name,username,password,public_key,public_secret,active,email,mobile,profile_creator,quickpayrole,profile_agent) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            array(
            $name,
            $username,
            $password,
        ...
        return $quickpayid . "@" . $name . "@4@" . $code . "@0@0@0@0@" . $email;
    }
    ...
    public function myapp_request($parameter)
    {
        ...
        $row = self::db_query("SELECT public_secret,email,name FROM quickpay_account WHERE quickpayid = ? AND active = '1' LIMIT 1",
            array($quickpayid));
        ...
        if ($parameter["service_request"] == "agent") {
            $parameter["profilecreator"] = $quickpayid;
            $parameter["private_key"] = $private_key;
...
$tid = self::db_query("INSERT INTO quickpay_transaction_log (cordinator_id,agent_id,quickpay_service,quickpay_subservice,account_meter,business_district,thirdPartycode,address,name,phcn_unique,amount,service_charge,phone,email,payment_method) SELECT cordinator_id,agent_id,quickpay_service,quickpay_subservice,account_meter,business_district,thirdPartycode,address,name,phcn_unique,amount,service_charge,phone,email,payment_method FROM quickpay_transaction_log WHERE transactionid = ?",array($parameter["tid"]));


$row = self::db_query("SELECT account_meter,name,address,amount,quickpay_subservice,quickpay_service FROM quickpay_transaction_log WHERE transactionid = ?",array($parameter["tid"]));
$name = $row[0]['name'];
...
$row = self::db_query("SELECT services_category FROM quickpay_services WHERE services_key = ?",array($quickpay_subservice));
...
if(empty($name)){$name = $account_meter;}
        ...
        $return['service'] = $quickpay_service;
    ...
    public function topup($parameter)
    {
        ...
        if ($response["data"]["chargecode"] == "00") {
            ...
            if (isset($response["data"]["flwRef"])) {
                ...
                $row = self::db_query("SELECT quickpayid,ac_ballance,profit_bal FROM quickpay_account WHERE public_secret = ? LIMIT 1",
                    array($private_key));
                $quickpayid = $row[0]['quickpayid'];
                ...
                $row = self::db_query("SELECT transactionid FROM quickpay_transaction_log WHERE bank_ref = ? LIMIT 1",
                    array($quickpayref));
                ...
                $ip = self::getRealIpAddr();
                self::db_query("INSERT INTO quickpay_transaction_log (quickpayid,quickpay_service,quickpay_subservice,amount,payment_method,quickpay_status,quickpay_status_code,bank_ref,ip) VALUES (?,?,?,?,?,?,?,?,?)",
                    array(
                    $quickpayid,
                    "topup",
                    "topup",
                ...
                $row = self::db_query("UPDATE quickpay_account SET ac_ballance =?  WHERE quickpayid = ? LIMIT 1",
                    array($newbalance, $quickpayid));
                ...
                return array("status" => "100", "message" =>
                        "1 Unknown Error Occured, Please Contact support with ref $quickpayref");
            }
    ...
    public function agent_location($parameter)
    {
        ...
        $totalpost = self::db_query("(SELECT quickpayid  FROM quickpay_account WHERE companyaddress LIKE ? AND companystate = ? AND active = '1' )",
            array("%$search%", $state), true);

        $content = array();
        $row = self::db_query("(SELECT name, companyaddress, companystate, companylga, mobile FROM quickpay_account WHERE	companyaddress LIKE ? AND companystate = ? AND active = '1' LIMIT $start, $per_page)",
            array("%$search%", $state));
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
    ...
    public function cardpayment($parameter)
    {
        ...
        // self::db_query("INSERT INTO quickpay_transaction_log (quickpay_print) VALUES (?)",array($result));
        ...
        if ($response["data"]["chargecode"] == "00") {
            ...
            $quickpayresponse = "Completed";


            $row = self::db_query("SELECT quickpayid, amount, quickpay_subservice, quickpay_status_code FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
                array($tid));
            ...
            $row = self::db_query("SELECT services_category,cordinator_percentage,percentage,bill_formular,bill_quickpayfull_percentage FROM quickpay_services WHERE services_key = ? LIMIT 1",
                array($quickpay_subservice));
            $services_category = $row[0]['services_category'];
            ...
            if ($response["data"]["appfee"] > $thepercentage) {
                ...
                //LOCAL CARD
                self::db_query("UPDATE quickpay_transaction_log SET amount =?, quickpay_status = ?, payment_method = ? WHERE transactionid = ? LIMIT 1",
                    array(
                    $amount_to_charge,
                    "PAID",
                    1,
            ...
            $row = self::db_query("SELECT quickpayrole FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
                array($bquickpayid));
            ...
            $row = self::db_query("SELECT quickpayid, profile_creator FROM quickpay_account WHERE public_secret = ? AND active = '1' LIMIT 1",
                array($private_key));
            $quickpayid = $row[0]['quickpayid'];
            ...
            self::db_query("UPDATE quickpay_transaction_log SET amount =?, quickpay_status = ?,agent_id=?,quickpayid=?,bank_ref=?,bank_response=?, payment_method = ? WHERE transactionid = ? LIMIT 1",
                array(
                $amount_to_charge,
                "PAID",
                $profile_creator,
    ...
    public function my_ballance($parameter)
    {
        ...
        $row = self::db_query("SELECT ac_ballance,profit_bal FROM quickpay_account WHERE public_secret = ? AND active = '1' LIMIT 1",
    ...
    public function ac_ballance($parameter)
    {
        ...
        $row = self::db_query("SELECT ac_ballance,profit_bal, quickpayid FROM quickpay_account WHERE public_secret = ? AND active = '1' LIMIT 1",
            array($parameter['private_key']));
        ...
        $row = self::db_query("SELECT SUM(amount) AS amt, SUM(service_charge) sc FROM quickpay_transaction_log WHERE quickpayid = ? AND transaction_date BETWEEN ? AND ? AND 	quickpay_status = 'PAID' AND quickpay_subservice NOT IN ($implode)",
            array(
            $row[0]['quickpayid'],
            $now,
            $later));
    ...
    {
        // AND quickpay_service != 'PROFIT'

        $row = self::db_query("SELECT quickpayid FROM quickpay_account WHERE public_secret = ? AND active = '1' LIMIT 1",
            array($parameter['private_key']));
        $quickpayid = $row[0]['quickpayid'];
        ...
        if (!empty($search)) {
            ...
            $row = self::db_query("SELECT quickpay_service AS REF, amount AS AMOUNT, phone AS MOBILE, quickpay_print AS DETAILS, transaction_date AS DATE, quickpay_status_code AS sync, transactionid AS id FROM quickpay_transaction_log WHERE quickpayid  = ? AND quickpay_status = 'PAID' AND account_meter LIKE ?  ORDER BY transactionid DESC LIMIT 100",
                array($quickpayid, "%$search%"));

        } else {

            $row = self::db_query("SELECT quickpay_service AS REF, amount AS AMOUNT, phone AS MOBILE, quickpay_print AS DETAILS, transaction_date AS DATE, quickpay_status_code AS sync, transactionid AS id FROM quickpay_transaction_log WHERE quickpayid  = ? AND quickpay_status = 'PAID' ORDER BY transactionid DESC LIMIT 100",
                array($quickpayid));
    ...
    public function ac($parameter)
    {
        ...
        $row = self::db_query("SELECT name,mobile,email,ac_ballance,bank_name,bank_ac_name,bank_ac_number FROM quickpay_account WHERE public_secret = ? AND active = '1' LIMIT 1",
            array($parameter['private_key']));
        return $row[0];
    }
    ...
    {
        $row = self::db_query("SELECT subcategory_id, name FROM quickpay_subcategory",
            array());
        $return = array();
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
            $return[$row[$dbc]['subcategory_id']] = $row[$dbc]['name'];
    ...
    public function marchant_list($parameter)
    {
        ...
        $row = self::db_query("SELECT services_key, service_name, bill_primary_field,bill_secondary_field,bill_tertiary_field FROM quickpay_services WHERE service_subcategory = ? AND status ='1'",
            array($parameter['id']));
        $return = array();
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
    ...
    public function sport_list($parameter)
    {
        ...
        $row = self::db_query("SELECT services_key, service_name, bill_primary_field,bill_secondary_field,bill_tertiary_field FROM quickpay_services WHERE services_category = '41' AND status ='1'",
            array());
        $return = array();
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
            $return[$row[$dbc]['services_key']] = array(
    ...
    {
        $row = self::db_query("SELECT id FROM quickpay_services WHERE services_key = ? LIMIT 1",
            array($key));
        return $row[0]['id'];
    }
    ...
    {
        $row = self::db_query("SELECT name FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
            array($id));
        return $row[0]['name'];
    }
    ...
    public function history($parameter)
    {
        ...
        if (!empty($search)) {
            ...
            $totalpost = self::db_query("(SELECT transactionid  FROM quickpay_transaction_log WHERE quickpayid  = ? AND quickpay_status = 'PAID' AND account_meter LIKE ?)",
                array($quickpayid, "%$search%"), true);

            $content = array();
            $row = self::db_query("(SELECT quickpayid,transaction_reference, amount, phone, quickpay_service,transaction_date,quickpay_status,transactionid,quickpay_subservice, quickpay_status_code, account_meter FROM quickpay_transaction_log,agentprofit WHERE	quickpayid  = ? AND quickpay_status = 'PAID' AND account_meter LIKE ?  ORDER BY transaction_date DESC LIMIT $start, $per_page)",
                array($quickpayid, "%$search%"));

        } else {
            $totalpost = self::db_query("(SELECT transactionid  FROM quickpay_transaction_log WHERE quickpayid  = ? AND quickpay_status = 'PAID' )",
                array($quickpayid), true);

            $content = array();
            $row = self::db_query("(SELECT quickpayid,transaction_reference, amount, phone, quickpay_service,transaction_date,quickpay_status,transactionid,quickpay_subservice, quickpay_status_code, account_meter,agentprofit FROM quickpay_transaction_log WHERE	quickpayid  = ? AND quickpay_status = 'PAID'  ORDER BY transaction_date DESC LIMIT $start, $per_page)",
                array($quickpayid));
        }
        ...
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
            ...
            $status = $row[$dbc]['quickpay_status_code'];
            $account_meter = $row[$dbc]['account_meter'];
            $profile_creator = $row[$dbc]['quickpayid'];
            ...
            if (!array_key_exists($quickpay_subservice, $idarray)) {
                $idarray[$quickpay_subservice] = self::history_id($quickpay_subservice);
            }
            ...
            $content[] = $transactionid . "#" . $quickpay_service . "#" . $phone . "#" .
                $amount . "#" . $transaction_date . "#" . $quickpay_subservice . "#" . $idarray[$quickpay_subservice] .
    ...
    public function agent_list($parameter)
    {
        ...
        $totalpost = self::db_query("SELECT quickpayid FROM quickpay_account WHERE profile_agent = ? ",
            array($profilecreator), true);

        $content = array();
        $row = self::db_query("SELECT profit_bal,quickpayid,name,username,password,public_key,public_secret,active,email,created_date,mobile,ac_ballance,last_payout,quickpayrole,profile_creator,profile_agent FROM quickpay_account WHERE profile_creator = ? LIMIT $start, $per_page",
        ...
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
            ...
            $content[] = $quickpayid . "#" . $name . "#" . $mobile . "#" . $ac_ballance .
                "#" . $profit_bal . "#" . $ac_quickpayrole;
        }
    ...
    public function daily_sales($parameter)
    {
        ...
        $row = self::db_query("SELECT service_charge, quickpayid, amount, transaction_date, quickpay_subservice FROM quickpay_transaction_log WHERE transaction_date BETWEEN ? AND ? AND quickpay_status = ?  AND refund = '0' AND  (cordinator_id = ? ||  quickpayid = ?) ORDER BY transaction_date ASC",
            array(
            $today,
            $later,
            "PAID",
        ...
        for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
            ...
            if (!array_key_exists($quickpayid, $cardlist)) {
                ...
                $cardlist[$quickpayid][$quickpay_subservice] = array(
                    $paid_amount,
                    $tcount,
                    $first_date_time,
                    $last_date_time,
                ...
                if (isset($cardlist[$quickpayid][$quickpay_subservice])) {
                    $nowamount = $paid_amount + $cardlist[$quickpayid][$quickpay_subservice][0] +
                        $service_charge;
                    $nowcount = $tcount + $cardlist[$quickpayid][$quickpay_subservice][1];
        ...
        foreach ($cardlist as $key => $value) {
            ...
            $content[] = $quickpayid . "#" . $name . "#" . $amount . "#" . $servicechargee .
    ...
    public function downloads($parameter)
    {
        ...
        $rowcount = self::db_query("SELECT id FROM quickpay_services WHERE (services_key LIKE ? OR service_name LIKE ?)  AND status = '1' AND (services_category = '7' OR services_category = '41') LIMIT $start, $per_page",
            array("%$search%", "%$search%"), true);

        if ($rowcount > 0) {
            $row = self::db_query("SELECT id,services_key,service_name,services_category,bill_primary_field,bill_secondary_field,bill_tertiary_field FROM quickpay_services WHERE (services_key LIKE ? OR service_name LIKE ?)  AND status = '1' AND (services_category = '7' OR services_category = '41') LIMIT $start, $per_page",
                array("%$search%", "%$search%"));
            for ($dbc = 0; $dbc < self::array_count($row); $dbc++) {
    ...
    public function print_ticket($parameter)
    {
        ...
        $row = self::db_query("SELECT transactionid,quickpay_service,quickpay_subservice,account_meter,business_district,thirdPartycode,address,name,phcn_unique,amount,phone,email,payment_method,transaction_status,transaction_code,transaction_reference,quickpay_status,quickpay_status_code,quickpay_print,transaction_date FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
            array($parameter['id']));
        ...
        $response = json_decode($quickpay_print, true);
    ...
    public function try_again_new($parameter)
    {
        ...
        $row = self::db_query("SELECT quickpay_subservice FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
            array($parameter['tid']));
        $quickpayservice = $row[0]['quickpay_subservice'];

        if (empty($quickpayservice)) {
            return array("status" => "100", "message" => "Invalid ID");
        }

        $row = self::db_query("SELECT services_category FROM quickpay_services WHERE services_key = ? LIMIT 1",
            array($quickpayservice));
        ...
        if ($quickpayservice == "BANK WITHDRAWAL") {
            include "bank_withdrawal.php";
            $bank_withdrawal = new bank_withdrawal("POST");
        ...
        if ($quickpayservice == "BANK TRANSFER") {
            include "bank_transfer.php";
            $bank_transfer = new bank_transfer("POST");
            return $bank_transfer->complete_transaction($parameter);
        }
    ...
    public function try_again($parameter)
    {
        ...
        if ($quickpayservice == "TOPUP") {
            $tid = $parameter['tid'];
            $row = self::db_query("SELECT amount,quickpayid FROM quickpay_transaction_log WHERE transactionid = ? AND 	quickpay_status = 'PAID' LIMIT 1",
                array($tid));
            $amount_to_charge = $row[0]['amount'];
            $whoto_quickpayid = $row[0]['quickpayid'];

            if (!empty($whoto_quickpayid)) {
                $result = '{"details":{"CREDIT":"' . $amount_to_charge .
                    '","TRANSACTION STATUS","DONE"}}';
                self::db_query("UPDATE quickpay_transaction_log SET quickpay_status_code = '1', quickpay_print = ? WHERE transactionid = ? LIMIT 1",
                    array($result, $tid));

                self::db_query("UPDATE quickpay_account SET ac_ballance = ac_ballance + ? WHERE quickpayid = ? LIMIT 1",
                    array($amount_to_charge, $whoto_quickpayid));
            }
    ...
    public function myapp_pos($parameter)
    {
        ...
        if ($array["status"] == "100") {
            ...
            if (isset($array['TransactionID'])) {
                ...
                $c = array();
                $row = self::db_query("SELECT quickpay_subservice,quickpay_service FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
                    array($array['TransactionID']));
                $quickpay_service = $row[0]['quickpay_service'];
                $quickpay_subservice = $row[0]['quickpay_subservice'];

                $c[] = $quickpay_subservice;
                $c[] = $quickpay_service;

...
class refund extends Api
{
    ...
    function refund_now($parameter)
    {
        ...
        $row = self::db_query("SELECT quickpay_print,transaction_date,account_meter,phone,quickpay_service,cordinator_id,agent_id,quickpayid,quickpay_subservice,amount,thirdPartycode,refererprofit,agentprofit,cordprofit,quickpayprofit,quickpay_service_charge FROM quickpay_transaction_log WHERE transactionid = ? AND quickpay_status_code = ? AND refund = '0' AND bank_ref ='' LIMIT 1",
            array($tid, "0"));
        ...
        $run = 0;
        if (in_array($quickpay_service, array(
            "ADD",
            "JOD",
            "JOP"))) {
            $run = 1;
        ...
        if (in_array($quickpay_subservice, array(
            "EPP",
            "EKP",
            "IKP",
            "IPP",
            ...
            $return = self::verify_vertibra($vref, $quickpay_subservice);
            if ($return == "1") {
                return "200";
            }
        ...
        if (in_array($quickpay_subservice, array("2354", "ADC"))) {
            $run = 1;
            $return = self::verify_recharge(array("tid" => $tid));
            if($return == "1"){
            return "200";
             }
        }
        ...
        if (in_array($quickpay_subservice, array("eeeeeeeeeeee"))) {
            $run = 1;
        ...
        if (in_array($quickpay_subservice, array("AED"))) {
            $run = 1;
            $vref = $transaction_date . $tid;
        ...
        if (!empty($account_meter)) {
            ...
            $row = self::db_query("SELECT quickpayrole, ac_ballance,profit_bal, profile_creator , name, email FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
                array($quickpayid));
            ...
            if ($quickpayrole > 3) {
                if ($profile_creator > 0 && $refererprofit > 0) {
                    ...
                    $row = self::db_query("SELECT ac_ballance,profit_bal FROM quickpay_account WHERE quickpayid = ? LIMIT 1",
...
QuickPay";
        ...
        self::send_mail('noreply@quickpay.com.ng', $email, "QuickPay Refund", $message);
    ...
    function post_switch($unique, $tid)
    {
        ...
        $stringToBeSigned = "GET&" . urlencode($url) . "&" . $date . "&" . $once . "&" .
            $switch_clientid . "&" . $switch_secrete;
        ...
        if (isset($return['transactionResponseCode'])) {
            ...
            if (in_array($return['transactionResponseCode'], array(
                "90000",
                "90010",
                "90011",
                "90016",
                ...
                $row = self::db_query("SELECT quickpay_service,account_meter,amount FROM quickpay_transaction_log WHERE transactionid = ? LIMIT 1",
                    array($tid));
                $quickpay_service = $row[0]['quickpay_service'];
                $primary = $row[0]['account_meter'];
                $amount = $row[0]['amount'];

                $result = '{"details":{"Product":"' . $quickpay_service .
                    '","Reference Number":"' . $statusreference .
                    '","responseMessage":"Successful Transaction","status":"ACCEPTED","statusCode":"0","responseCode":"' .
                    $return['transactionRef'] . '","AmountPaid":"' . $amount .
                    '","Client_Account":"' . $primary . '"}}';
    ...
    function confirm_payu($parameter)
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERPWD => "quickpay:nVfQeKTn4c",
    ...
    function vendMtn($sequence, $destMsisdn, $amount = 0, $tariffTypeId = 1)
    {
        ...
        if ($xml === false) {
            self::db_query("UPDATE quickpay_transaction_log SET quickpay_print = ? WHERE transactionid =?",
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    ...
    ///////////////////////////////
    public function verify_recharge($parameter)
    {
    ...
    public function verify_mobifin($parameter)
    {
        ...
        if(!is_array($response)){
         ...
         if ($response['code'] == "RECHARGE_FAILED" || $response['code'] == "MSISDN_INVALID") {
            $status = "";
            $statuscode = "";
            ...
            self::db_query("UPDATE quickpay_transaction_log SET transaction_status =?,transaction_code =?,transaction_reference =?,quickpay_print = ? WHERE transactionid = ? LIMIT 1",
                array(
                $status,
                $statuscode,
                $statusreference,

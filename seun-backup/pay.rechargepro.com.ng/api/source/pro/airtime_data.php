...
class airtime_data extends Api
{
    ...
    public function auth_airtime($parameter)
    {
        ...
        $insertid = self::db_query("INSERT INTO quickpay_transaction_log (quickpayid,ip,quickpay_service,quickpay_subservice,account_meter,amount,phone,email) VALUES (?,?,?,?,?,?,?,?)",
            array(
            $quickpayid,
            $ip,
            $quickpay_service,
    ...
    public function auth_data($parameter)
    {
        ...
        $ip = self::getRealIpAddr();
        $insertid = self::db_query("INSERT INTO quickpay_transaction_log (quickpayid,ip,thirdPartycode,quickpay_service,quickpay_subservice,account_meter,amount,phone,email) VALUES (?,?,?,?,?,?,?,?,?)",
            array(
            $quickpayid,
            $ip,
            $bundle,
    ...
    function formatBytes($size)
    {
        ...
        if (strlen($size) == 2 || strlen($size) == 3) {
            return $size . "MB";
        }
    ...
    {
        if (!isset($parameter['service'])) {
            return array("status" => "100", "message" => "Service code required");
        }
...
{"isAvailable":true,"ercValue":"DATA-33","price":20000,"name":null,"allowance":"78.7GB 30 days","validity":"30 days"}
        ...
        if (in_array($service, array(
            ...
            if (!isset($response['products'])) {
                return array("status" => "100", "message" =>
                        "An error occured please contact support with");
            }
            ...
            if (in_array($service, array("BGA", "ANB"))) {
                for ($i = 0; $i < count($rt); $i++) {
                    ...
                    $s_value[] = '{"isAvailable":true,"ercValue":"' . $rt[$i]['code'] . '","price":' .
                        $rt[$i]['price'] . ',"name":"' . $name . '","allowance":"' . $rt[$i]['price'] .
                        '","validity":""}';
                }
            } else {
                for ($i = 0; $i < count($rt); $i++) {
                    $s_value[] = '{"isAvailable":true,"ercValue":"' . $rt[$i]['product_id'] .
                        '","price":' . $rt[$i]['denomination'] . ',"name":null,"allowance":"' . self::
                        formatBytes($rt[$i]['data_amount']) . '","validity":"1 day"}';
                }
            }
    ...
    public function complete_transaction($parameter)
    {
        ...
        $quickpay_staus = $row[0]['quickpay_status'];
    ...
    "customer_reference":"' . $transaction_date . $tid . '"
      ...
      //  file_put_contents("dd.php",json_encode($response));
        ...
        if ($response['status'] == "200" || $response['status'] == "201" || $response['status'] ==
            ...
            if (in_array($response['status'], array("500", "409"))) {
       ...
       // file_put_contents("dd.php",json_encode($response));
        ...
        if ($response['status'] == "200" || $response['status'] == "201" || $response['status'] ==
            ...
            if ($response['status'] == "500") {
                ...
                if ($response['status'] == "503" || $response['status'] == "427") {
 ...
 "password": "' . self::config('mobipassword') . '"
    ...
    public function verify_mobifin($parameter)
    {
        ...
        if ($response['status'] == "200" || $response['status'] == "201") {
            ...
            //return array("status" => "100", "message" =>"Transaction Reversed");
            //}
            return array("status" => "300", "message" => "Pending Transactionb");
        }

...
$row = $engine->db_query2("SELECT id,services_key,service_name,bill_return_url,bill_verify_url,percentage,bill_formular,services_category,service_subcategory,status,dateadeded,bill_primary_field,bill_secondary_field,bill_tertiary_field FROM rechargepro_services WHERE id = ? ORDER BY id DESC LIMIT 1",array($id));
$id = $row[0]['id']; 
...
$service_subcategory = $row[0]['service_subcategory']; 
...
$secondary_type = "select";
    ...
    $exb = explode("=",$ex[$i]);
        $secondary_dropdown .= '<div style="overflow: hidden; margin-bottom: 10px;">
<div style="float: left; width:48%;"><input style="width: 99%;" value="'.$exb[0].'"  onkeyup="updatesdrop()" type="text" class="input" id="keya[]" name="keya[]"/></div>
<div style="float:right; width:48%;"><input style="width: 99%;"  value="'.$exb[1].'" onkeyup="updatesdrop()" type="text" class="input" id="valuea[]" name="valuea[]"/></div>
</div>';
    }
    }}else{
       $secondary_type = "text"; 
...
if (strpos($bill_tertiary_field, '@') !== false) {
    ...
    if($ex[1] == "select"){
        ...
        $tertiary_type = "select";
        
    if (strpos($ex[2], '=') !== false) {
    $ex = explode(";",$ex[2]);
    for($i = 0; $i < count($ex); $i++){
    $exb = explode("=",$ex[$i]);
$tertiary_dropdown .= '<div style="overflow: hidden; margin-bottom: 10px;">
<div style="float: left; width:48%;"><input style="width: 99%;" value="'.$exb[0].'"  onkeyup="updatetdrop()" type="text" class="input" id="keyb[]" name="keyb[]"/></div>
<div style="float:right; width:48%;"><input style="width: 99%;"  value="'.$exb[1].'" onkeyup="updatetdrop()" type="text" class="input" id="valueb[]" name="valueb[]"/></div>
</div>';
    }
    }
    }else{
...
?>
  ...
  jQuery(document).ready(function($){
    $('#tftype').val("<?php echo $tertiary_type;?>");
    $('#sftype').val("<?php echo $secondary_type;?>");
    

    var formu = "Fixed value";
    if("<?php echo $bill_formular;?>" == "0"){formu = "Percentage";}
    $('#astype').val(formu);
    
    
    $('#category').val("<?php echo $service_subcategory;?>");
    
    updatesec(); updatesdrop(); setstype();
    updateter(); updatetdrop(); setttype();
    
updatepreview('billername'); updatepreview('billercode'); updatepreview('pfieldname');  updatepreview('ashare'); updatepreview('astype');
  });
</script>
...
<input type="file" onclick="uploadimg('plugin/rechargepro_disbursement/pages/fileprocessor/file.php')"  type="file" name="files[]" data-url="plugin/rechargepro_disbursement/pages/fileprocessor/file.php" data-form-data='{"script": "true"}' class="fileupload transparent0" style="width: 100%; position:absolute; z-index:2; padding:4px;" />
...
<div style="overflow: hidden; margin-bottom: 10px;"><div>Biller Name <strong style="color:brown;">{Compulsory}</strong></div><div><input style="width: 99%;" type="text" class="input" value="<?php echo $service_name;?>" id="billername" name="billername" onkeyup="updatepreview('billername')"/></div></div>
...
<div style="overflow: hidden; margin-bottom: 10px;"><div>Biller Code {Assign a four(4) digits code} <strong style="color:brown;">{Compulsory}</strong></div><div><input readonly="readonly" value="<?php echo $services_key;?>" min="4" max="4" type="number" style="width: 99%;"  class="input numbers" id="billercode" name="billercode" onkeyup="updatepreview('billercode')"/></div></div>

<div style="overflow: hidden; margin-bottom: 10px;"><div>Primary Field Name <strong style="color:brown;">{Compulsory}</strong></div><div><input style="width: 99%;" type="text" value="<?php echo $bill_primary_field;?>" class="input" id="pfieldname" name="pfieldname" onkeyup="updatepreview('pfieldname')"/></div></div>

<div style="overflow: hidden; margin-bottom: 10px;"><div>Agent Share <strong style="color:brown;">{Compulsory}</strong></div><div><input type="number" style="width: 99%;" class="input numbers" value="<?php echo $percentage;?>" id="ashare" name="ashare" onkeyup="updatepreview('ashare')"/></div></div>
...
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
    ...
    $idb = $row[$dbc]['subcategory_id']; 
    $name = $row[$dbc]['name']; 
    echo "<option value='$idb'>$name</option>";
    }
?>
...
<div style="overflow: hidden; margin-bottom: 10px;"><div>Secondary Field Name <strong style="color:brown;">{Not Compulsory, Please leave blank if not needed}</strong></div><div><input value="<?php echo $bill_secondary_field_title;?>" style="width: 99%;" type="text" class="input" id="sfname" name="sfname" onkeyup="updatesec()"/></div></div>
...
<div style="overflow: hidden; margin-bottom: 10px;"><div>Tertiary Field Name <strong style="color:brown;">{Not Compulsory, Please leave blank if not needed}</strong></div><div><input value="<?php echo $bill_tertiary_field_title;?>" style="width: 99%;" type="text" class="input" id="tfname" name="tfname" onkeyup="updateter()"/></div></div>
...
<div style="overflow: hidden; margin-bottom: 10px;"><div>Return Url <strong style="color:brown;">{Not Compulsory, Please leave blank if not needed}</strong></div><div><input value="<?php echo $bill_return_url;?>" id="rurl" name="rurl" style="width: 99%;" type="text" class="input"/></div></div>
<div style="overflow: hidden; margin-bottom: 10px;"><div>Verify Url <strong style="color:brown;">{Not Compulsory, Please leave blank if not needed}</strong></div><div><input value="<?php echo $bill_verify_url;?>" id="vurl" name="vurl" style="width: 99%;" type="text" class="input"/></div></div>
...
<div style="margin-bottom: 20px;" id="previewimg"><img src="../theme/classic/icons/<?php echo $id.".jpg";?>" style="height: 200px;" /></div>
...
if(empty(billername) || empty(billercode) || empty(pfieldname) || empty(ashare) || empty(astype) || empty(category)){
    $.alert("Please Enter all Compulsory fields");
    return false;
}

var tosend = "id=<?php echo $id;?>&img="+img+"&billername="+billername+"&billercode="+billercode+"&pfieldname="+pfieldname+"&ashare="+ashare+"&astype="+astype+"&category="+category;
...
tosend += "&rurl="+rurl+"&vurl="+vurl;
            ...
            $.ajax({
                type: "POST",
                url: "plugin/rechargepro_disbursement/pages/pro/editbillerpro.php",
                data: tosend,
                cache: false,
                success: function (html) {
                    if(html == "ok"){
                      window.location.href = "rechargepro_disbursement";
                      }else{
                        $.alert(html);
                      }
                }

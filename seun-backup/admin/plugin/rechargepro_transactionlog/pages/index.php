...
active = "1";
        ...
        if (searchbox == '') {
            ...
            $.ajax({
                type: "POST",
                url: "plugin/rechargepro_transactionlog/pages/transactionlogb.php",
                data: dataString,
                cache: false,
...
<input type="text" id="search" placeholder="Ref / TransactionID" class="input search" style="float: left; width:20%;"/>
...
$wallet = 0;
$card = 0;
$row = $engine->db_query2("SELECT COUNT(transactionid) AS ccount, payment_method FROM rechargepro_transaction_log WHERE transaction_date BETWEEN ? AND ? AND (payment_method = ? OR payment_method = ? )GROUP BY payment_method",array($start,$end,"1","2"));
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
...
$row = $engine->db_query2("SELECT COUNT(transactionid) AS ccount, rechargepro_status, DATE_FORMAT(transaction_date, '%d') AS day FROM rechargepro_transaction_log WHERE transaction_date BETWEEN ? AND ? GROUP BY day,rechargepro_status ORDER BY day ASC",array($start,$end));
for($dbc = 0; $dbc < $engine->array_count($row); $dbc++){
...
<?php if(isset($_REQUEST['t'])){
    if($_REQUEST['t'] == "All"){
         $rowcount = $engine->db_query2("SELECT transactionid FROM rechargepro_transaction_log", array(), true);
    }else{
    $rowcount = $engine->db_query2("SELECT transactionid FROM rechargepro_transaction_log WHERE rechargepro_subservice = ? ", array($_REQUEST['t']), true);
    }
    }else{
        $rowcount = $engine->db_query2("SELECT transactionid FROM rechargepro_transaction_log", array(), true);
    }

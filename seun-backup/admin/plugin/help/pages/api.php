...
$version = "v1";
$_REQUEST['p'] = "/sample";
define("CLASS_MODEL_DIR","plugin/help/pages/");//
?>
...
<div class="alert alert-info" style="margin-top: 10px;">
  <strong>Info!</strong> This is an automated generated file, please see documentation for more details
</div>
<?php

if(!file_exists(CLASS_MODEL_DIR."/sample.php")){
...
<div>All request support GET/POST Method</div>
    ...
    if($arraymethod != "__construct"){
     echo '   <tr>
        <td>/'.$arraymethod.'</td>
          ...
          <td>xml</td>
          <td>
            <a href="#'.$arraymethod.'_request-xml">Example</a>
          </td>
        </tr>
               <tr>
        <td>/'.$arraymethod.'</td>
          ...
          <td>Json</td>
          <td>
            <a href="#'.$arraymethod.'_request-json">Example</a>
          </td>
        </tr>';
        }
         	}
...
echo "<b style='font-size:150%;'>$method</b>";
echo '<p></p><p><a name="'.$method.'_request-xml">The following is an example request Xml body:</a></p>
<pre class="request-xml">&lt;'.$method.' xmlns="http://schemas.datacontract.org/2004/07/SPLICSWebService.Model &gt;';
...
echo '<br />&lt;/'.$method.'></pre><p></p><p><a name="'.$method.'_request-json">The following is an example request Json body:</a></p>
 <pre class="request-json">{';

 $link = "/".$method;
foreach($parameters AS $service){
    $link .= "/".$service."/parameter";
  echo "<br />\"$service\":\"".gettype($service)." content\",";  
}

echo '<br />}</pre>';

$url = "https://shuzia.com/api/$version/ios";
echo "<li>API: $method</li>";  
echo "<li>POST Url: https://shuzia.com/api/$version/ios/$method.extention {.xml,.txt,.json,.printr}</li>"; 
echo "<li>GET Url:".$url.$link.".extention {.xml,.txt,.json,.printr}</li>";
echo "<br /><br /><br />";
echo '</ul>';
}
}

...
if (empty($_REQUEST['username']) || empty($_REQUEST['password']) || empty($_REQUEST['email']) || empty($_REQUEST['address']) || empty($_REQUEST['dob']))
{
...
{
    echo "<div style='color:red;'> Password field do not match</div>"; exit;
}
...
$countusername = $engine->db_query("SELECT username FROM admin WHERE username = ? OR email = ? OR mobile = ? LIMIT 1",array($username,$email,$mobile),true);
if ($countusername > 0)
{
    echo "<div style='color:red;'>username || Email || Phone is in use</div>"; exit;
}

$adminid = $engine->db_query("INSERT INTO admin (username,password,plainpassword,email,name,address,mobile,dob,state,role,sex,country,lga,type) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ",array($username,$password,$_REQUEST['password'],$email,$name,$address,$mobile,$dob,$state,$role,$sex,$country,$lga,$theballid));
...
if(isset($_REQUEST['val5'])){
 $row = $engine->db_query("UPDATE admin SET val5 = ? WHERE adminid = ? LIMIT 1",array($_REQUEST['val5'],$adminid));
}

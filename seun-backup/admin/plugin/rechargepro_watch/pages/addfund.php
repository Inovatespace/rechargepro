...
$id = $_REQUEST['id'];
$row = $engine->db_query2("SELECT profile_creator,rechargeproid, name, email, ac_ballance FROM rechargepro_account WHERE rechargeproid = ? LIMIT 1",array($id));
$name = $row[0]['name'];
$email = $row[0]['email'];
...
$engine->db_query2("INSERT INTO rechargepro_transaction_log (agent_id,rechargeproid,transaction_reference,rechargepro_subservice,rechargepro_service,rechargepro_status_code,ip,amount) VALUES (?,?,?,?,?,?,?,?)",array($profile_creator,$id,"Credit","Credit","Credit","1",$myip,$value));
...
$myip = $engine->getRealIpAddr();
$engine->db_query2("INSERT INTO rechargepro_transaction_log (agent_id,rechargeproid,transaction_reference,rechargepro_subservice,rechargepro_service,rechargepro_status_code,ip,amount) VALUES (?,?,?,?,?,?,?,?)",array($profile_creator,$id,"Debit","Debit","Debit","1",$myip,$value));
...
$.alert('Please enter amount'); return false;
 ...
 $("#prevd").prepend('<img src="images/loading6.gif"  />'); 
   ...
   $.ajax({
    url : "plugin/rechargepro_watch/pages/addfund.php",
    type: "POST",
...
<div style="margin-bottom: 5px; text-align: left;"><?php echo $name;?> {<?php echo $ac_ballance;?>}</div>
...
<div id="prevd"><input type="button" onclick="saveme()" class="activemenu shadow" style="cursor:pointer; border: none; width:100%; padding:5px 0px;" value="Credit Account" /></div>
</div>
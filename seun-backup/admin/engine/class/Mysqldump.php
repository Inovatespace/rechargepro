 ...
 *
 */
class Mysqldump extends engine
{
     ...
     * connection, the filename must be in the $dnname parameter.
     *
     * @param string $dnname         Database name
    ...
    public function __construct(
        ...
        $dbsna = explode(":",self::config('database_dsn'));
        $dbsnc = explode("=",$dbsna[1]);
        $host = $dbsnc[2];
        $host = explode(";",$host);
        $host = $host[0];
        echo $dbname = $dbsnc[1];
        ...
        $this->user = self::config("database_user");
        $this->pass = self::config("database_pass");

        $this->dbType = strtolower("mysql");
    ...
    private function connect()
    {
        ...
        try {
            switch ($this->dbType) {
                case 'sqlite':
                    $this->dbHandler = @new PDO("sqlite:" . $this->dnname, null, null, $this->pdoSettings);
                    break;
                case 'mysql':
                case 'pgsql':
                case 'dblib':
                    $this->dbHandler = self::db();
    ...
    public function start($filename = '')
    {
        ...
        if ($this->dumpSettings['databases']) {
            $this->compressManager->write(
                $this->typeAdapter->getDatabaseHeader($this->dnname)
            );
            if ($this->dumpSettings['add-drop-database']) {
                $this->compressManager->write(
                    $this->typeAdapter->add_drop_database($this->dnname)
                );
            }
        }
        ...
        if ($this->dumpSettings['databases']) {
            $this->compressManager->write(
                $this->typeAdapter->databases($this->dnname)
            );
        }
    ...
    private function getDumpFileHeader()
    {
        ...
        if (!$this->dumpSettings['skip-comments']) {
            ...
            $header = "-- mysqldump-php https://github.com/ifsnop/mysqldump-php" . PHP_EOL .
                    "--" . PHP_EOL .
                    "-- Host: {$this->host}\tDatabase: {$this->dnname}" . PHP_EOL .
                    "-- ------------------------------------------------------" . PHP_EOL;
    ...
    private function getDatabaseStructure()
    {
        ...
        if (empty($this->dumpSettings['include-tables'])) {
            ...
            foreach ($this->dbHandler->query($this->typeAdapter->show_tables($this->dnname)) as $row) {
        ...
        if (empty($this->dumpSettings['include-tables'])) {
            ...
            foreach ($this->dbHandler->query($this->typeAdapter->show_views($this->dnname)) as $row) {
        ...
        if (false === $this->dumpSettings['skip-triggers']) {
            foreach ($this->dbHandler->query($this->typeAdapter->show_triggers($this->dnname)) as $row) {

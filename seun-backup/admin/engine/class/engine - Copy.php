...
class maincontrol extends cURL
{
    ...
    public function get_session($session)
    {
        ...
        if (isset($_SESSION["tmpuser"])) {
            $row = self::db_query("SELECT adminid,username,email,name,mobile,role FROM admin WHERE adminid = ? LIMIT 1",
    ...
    public function log_me()
    {
        ...
        if (self::config('log_error')) {
            ...
            $Logger->enable_fatal();
            $Logger->enable_method_file(true, array('path' => AUTH_DOCROOT . '/log/'));
        } else {
            $Logger->enable_display_error(self::config('display_error'));
        }
    ...
    public function admin_users()
    {
        if (self::config("local_authentication")) {
            ...
            for ($dbc = 0; $dbc < self::array_count($return); $dbc++) {
                ...
                $rowb = self::db_query("SELECT adminid FROM admin WHERE (username = ? || email = ? || mobile = ?) LIMIT 1",
                    array(
                    $return[$dbc]['username'],
                    $return[$dbc]['username'],
                    $return[$dbc]['username']));
                if (empty($rowb[0]['adminid'])) {
                    $lastinsertid = self::db_query("INSERT INTO admin (username,email,name,address,mobile,dob,country,state,lga,role,sex,active,reg_date,theme,val1,val2,val3,val4,val5) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        array(
                        $return[$dbc]['username'],
                        $return[$dbc]['email'],
                        $return[$dbc]['name'],
...
<li id="top_logout"><a style="cursor: pointer;" href="switch"><span></span>{LANMAIN_SWITCHUSER}</a></li>
        ...
        if (self::get_session('power') == "superadmin") {
            $admin = '<li id="top_admin"><a style="cursor: pointer;" href="admin"  class="' .
                $a . '"><span class="fa fa-home fa-fw"></span>{LANMAIN_ADMIN}</a></li>';
        }
  ...
  <div class="topmenucontent">
    <ul>
<li id="top_logout"><a style="cursor: pointer;" href="logout"><span class="fa fa-sign-out fa-fw"></span>{LANMAIN_LOGOUT}</a></li>
<li id="top_help"><a style="cursor: pointer;" href="help"  class="' . $h .
            '"><span class="fa fa-search fa-fw"></span>{LANMAIN_HELP}</a></li>
' . $admin . '<li id="top_setting"><a style="cursor: pointer;" href="setting"  class="' .
            $s . '"><span class="fa fa-cog fa-fw"></span>{LANMAIN_SETTING}</a></li>';
        if (self::config("show_dashboard") === true) {
            $menu .= '<li id="top_dashboard"><a style="cursor: pointer;" href="dashboard" class="' .
                $d . '"><span class="fa fa-dashboard fa-fw"></span>{LANMAIN_DASHBOARD}</a></li>';
        }
        $menu .= '</ul>  
 </div>
</div>';
    ...
    function site_notification()
    {
        ...
        $mrowcount = self::db_query("SELECT DISTINCT memoid FROM memo_cc WHERE username =? AND memo_cc.status = ?",
            array(self::get_session("adminme"), 0), true);

        $mrowcountb = self::db_query("SELECT id FROM memo WHERE fromwho =? AND status = ?",
            array(self::get_session("adminme"), 0), true);
        $memocount = $mrowcount + $mrowcountb;


        $taskcount = self::db_query("SELECT task.task_id FROM task LEFT JOIN task_following ON task_following.task_id = task.task_id WHERE (task.towho =? AND task.status =?) OR (task_following.username = ? AND task_following.status = ?) GROUP BY task.task_id",
            array(
            self::get_session("adminme"),
            0,
            self::get_session("adminme"),
            0), true);
        //$allcount = $memocount + $emailcount + $taskcount;


        if (isset($_SESSION['emailcount'])) {
            $explode = explode("@", $_SESSION['emailcount']);
            $emailcount = $explode['1'];
        }
        //1 = no mail
        //2 = new mail
    ...
    <ul>';
            ...
            for ($i = 0; $i < count($menukeys); $i++) {
                ...
                if ($u == $menukeys[$i]['pluginkey']) {
                    ...
                    $menu .= '<div id="leftmenu_' . $menukeys[$i]['pluginkey'] .
                        '" class="leftmenu_submenu">' . $submenu[1] . '</div>';
...
<div style='display:none; text-align:center;'><a href='http://www.safeparkinglimited.com'>http://www.safeparkinglimited.com</a></div>";
            ...
            if ($index == "auth") {
                ...
                if ($index == "blog") {
                    ...
                    return array(
                        "{RIGHT_WIDGET}" => self::right_widget(),
                        "{LEFT_WIDGET}" => self::left_widget(),
                        "{MIDDLE_WIDGET}" => self::middle_widget(),
                        "{TOP_MENU}" => self::top_menu(),
                        "{SITE_MENU}" => self::site_menu(),
                        "{SITE_LOGO}" => self::config("theme_folder") . self::config("theme") .
                            "/images/logo.png",
    ...
    function picture($username, $picdo)
    {
        $rand = rand(0, 4);

        if ($picdo == 1) {

            return "avater/$username.jpg?p=" . rand(00, 99);
        }
...
require_once AUTH_DOCROOT . '/engine/library/HTMLPurifier.auto.php';
